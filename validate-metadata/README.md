...
3. **OCI Reference Validation** (if `dynamicArtifact` starts with `oci://ghcr.io`):
   - **Tag Format**: The image tag should be `<image-tag-prefix><plugin version>`
...
- name: Validate Catalog Metadata
  ...
  with:
    ...
    image-tag-prefix: bs_1.42.5__  # Optional
```
...
| `image-tag-prefix` | No | `""` | Tag prefix for validating OCI tag format in `dynamicArtifact` |
...
INPUTS_IMAGE_TAG_PREFIX="prefix_" \
